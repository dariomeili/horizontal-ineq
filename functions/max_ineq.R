...
max_ineq <- function(.data, outcome, grouping, weights) {
  ...
  weights.max <- weights.max[order]
  p <- cumsum(weights.max)
  nu <- cumsum(weights.max * xMean_group.max)
  n <- length(nu)
  nu <- nu/nu[n]
  max_ggini <- sum(nu[-1] * p[-n]) - sum(nu[-n] * p[-1])
  
  return(list("group_shares" = share_group.max,
                "group_means" = xMean_group.max,
                "overall_mean" = xMean.max,
                "group_gini" = max_ggini, 
              "number_groups" = n_cat)) 
}  
